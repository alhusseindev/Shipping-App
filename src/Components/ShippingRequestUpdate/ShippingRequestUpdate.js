...
import './ShippingRequestUpdateStyles.css';
import axios from 'axios';


class ShippingRequestUpdate extends React.Component{
    constructor(props){
        super(props);
        this.state = {
            shipment: [
    ...
    updateShipment = (event) =>{
        if(window.confirm("Are you sure you want to update the selected shipment ?")) {
            axios.put(`http://localhost:8000/app/shipments/update/${this.state.shipment.id}`)
                .then((response) => {
                    let result = response.data;
                    window.alert(`Shipment Number: ${this.state.shipment.id} was updated successfully!`);

                }).catch((error) => {
                this.setState({errorMessage: this.state.errorMessage.push(error.response)});
    ...
    render() {
        return (
            <form className="parent-container" onSubmit={this.handleFormSubmission}>
                ...
                <button type="submit" className="updateShipment" onClick={() => this.updateShipment(this.state.shipment.id)} onChange={this.handleChange}>Update Shipment</button>
...
export default ShippingRequestUpdate;